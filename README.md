...
true` в стадии `rnnoise` (в шаблоне включён; без своей задержки: детектор на входе сети, усиление на
её выходе) или отдельная стадия `transient` перед NS (задержка `lookahead_ms`). После правки
конфига пункт меню «Reload config».
...
- Регрессия на эталонных записях (`[corpus]`): пары mic.wav + ref.wav из `bomboec-rec`
  ...
  `recordings/` (вне git; без них тест пропускается). Смеси `[[mix]]`: чистая речь плюс отдельно
  записанный шум или эхо, выход сравнивается с чистой речью (спектральное расстояние на речевых
  кадрах, доля провалов уровня, подавление на кадрах без речи): так видна порча голоса, которую
  RMS речи не показывает. Записать корпус:
  `pwsh scripts/record-corpus.ps1` (семь сценариев с подсказками, музыка через `bomboec-play
