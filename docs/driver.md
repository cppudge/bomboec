...
`install.ps1` ничего не делает, если `.sys` пакета совпадает с установленным (иначе см. ниже про
id endpoint'ов; `-Force` ставит всё равно), останавливает bomboec.exe (открытый кабель нельзя
выгрузить), перечисляет остальных держателей кабеля через `bomboec-sessions.exe` и без `-Force`
останавливается, пока их не закроют: иначе Windows оставляет старый драйвер до перезагрузки
(`CR_REMOVE_VETOED`). В конце сравнивает хеш `.sys` из DriverStore с пакетом. Для чужих машин test-signing не годится: нужна attestation-
...
(движок сам откажется брать микрофон кабеля при выходе в тот же кабель).

Идентификаторы endpoint'ов кабеля меняются при каждом реальном обновлении драйвера, даже без
удаления устройства: `devcon update` перезапускает devnode («Removing device and sub-tree»,
«Restarting device» в setupapi.dev.log), и через полсекунды endpoint builder удаляет старые
endpoint'ы (`Delete Device - SWD\MMDEVAPI\{0.0.1.00000000}.{...}`) и создаёт новые с другими
GUID; Windows хранит цепочку прежних id в свойствах нового endpoint'а, но приложениям их не
показывает. Приложения, помнящие микрофон по id (Discord, OBS), видят старый как отсутствующий:
там нужно заново выбрать «Microphone (bomboec Cable)». bomboec хранит id и имя и находит
устройство по имени сам. `install.ps1` печатает id до и после и предупреждает, если они
сменились, а неизменённый `.sys` не переустанавливает.
